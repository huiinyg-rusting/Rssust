...
pub fn get(para: HashMap<String, String>) -> Result<String, Error> {
    ...
    let url = format!("https://api.bilibili.com/x/space/coin/video?vmid={}", uid);
    ...
    for item in data {
        ...
        let pub_date = timestamp_to_rss(time);
    ...
    let channel = ChannelBuilder::default()
        .title(format!("{} 的 bilibili 投币视频", author))
        .link(format!("https://space.bilibili.com/{}", uid))
        .description(format!("{} 的 bilibili 投币视频", author))
        .items(item_vec)
        .build();

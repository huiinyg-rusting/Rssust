...
pub fn get(para:HashMap<String, String>) -> Result<String,Error>{
    ...
    let sort_reverse = match para.get("sortreverse") {
        Some(val) => val.parse::<bool>()
            .map_err(|_| anyhow::anyhow!("sortreverse 参数格式错误，应为 true 或 false"))?,
        None => true,
    };
    let sort_reverse_str = if sort_reverse { "1" } else { "0" };
    let page_num = para.get("pagenum")
        .and_then(|s| s.parse::<u32>().ok()) 
        .unwrap_or(1);                      

    let page_size = para.get("pagesize")
        .and_then(|s| s.parse::<u32>().ok())
        .unwrap_or(16);
    let url = format!("https://api.bilibili.com/x/polymer/web-space/seasons_archives_list?mid={}&season_id={}&sort_reverse={}&page_num={}&page_size={}",
    ...
    let json: Value = serde_json::from_str(fetch_reqwest_get_with_headers(url.as_str(), &headers)?.as_str())?;
    if json["code"] != 0 {
        return Err(anyhow!("请求失败，{}",json))
    }
    let author: String = serde_json::from_str::<Value>(fetch_reqwest_get_with_headers(&format!("https://api.bilibili.com/x/web-interface/view?aid={}",json.pointer("/data/aids/0").ok_or_else(|| anyhow!("bilibili_collection.rs在解析author时爬取信息出错"))?.to_string()), &headers)?.as_str())?.pointer("/data/owner/name").ok_or_else(|| anyhow!("bilibili_collection.rs在解析author时解析出错"))?.as_str().ok_or_else(|| anyhow!("owner/name 不是字符串"))?.to_owned();
    ...
    let ctitle: String = no_double_quotes(json.pointer("/data/meta/name").ok_or_else(|| 
        anyhow!("bilibili_collection.rs在解析channel标题时出错"))?.to_string());

    let channel = ChannelBuilder::default()
        .title(&ctitle)
        .link(format!("https://www.bilibili.com/video/{}",no_double_quotes(json.pointer("/data/archives/0/bvid").ok_or_else(|| anyhow!("bilibili_collection.rs在解析author时出错"))?.to_string())))
        .description(&ctitle)
        .items(item_vec)
        .build();
